...
function ConsultationsContent() {
  ...
  const toggleConsultationSelection = (consultationId) => {
    ...
    setSelectedId((current) => (current === consultationId ? '' : consultationId));
  };

  const renderConsultationDetail = (consultation) => (
    <div className="mt-2 space-y-5 rounded-3xl border border-premium-lilac/25 bg-white/85 p-4 shadow-premium-soft">
      ...
      <div className="rounded-3xl border border-premium-lilac/20 bg-white/75 p-4">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <Button
            ...
            variant="secondary"
            disabled={
              busyId === consultation.id || Boolean(getMessagingUnavailableReason(consultation))
            }
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              disabled={aiNotesLocked || aiBusyId === consultation.id || busyId === consultation.id}
            >
      ...
      {getConsultationStatus(consultation) === 'completed' ? (
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 text-sm text-amber-800">
            ...
            <p className="mt-1">
              This workspace contains sensitive patient data. Ensure notes are accurate and only
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(event) => updateNoteField(consultation.id, 'raw', event.target.value)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(event) => updateNoteField(consultation.id, 'outcome', event.target.value)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(event) => updateNoteField(consultation.id, 'plan', event.target.value)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(event) => updateNoteField(consultation.id, 'notes', event.target.value)}
            />
          </div>
        </div>
      )}
      ...
      <div className="rounded-3xl border border-dashed border-premium-lilac/30 bg-premium-pearl-tint/40 p-4">
        ...
        <p className="mt-2 text-sm text-premium-purple-plum/60">
          Clinical attachments will appear here when the dedicated consultation attach flow is
          connected.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Button
          disabled={busyId === consultation.id || getConsultationStatus(consultation) === 'completed'}
              ...
              {activeConsultations.length > 0 && (
                <div className="space-y-3">
                  ...
                  {activeConsultations.map((consultation, index) => {
                    const cardKey = consultation?.id || `consultation-${index}`;
                    const isSelected = Boolean(consultation?.id) && selectedId === consultation.id;

                    return (
                      <div key={cardKey} className="space-y-0">
                        <button
                          type="button"
                          onClick={() => toggleConsultationSelection(consultation?.id)}
                          aria-expanded={isSelected}
                          className={`w-full rounded-3xl border p-4 text-left transition-all ${isSelected ? 'border-premium-purple-plum bg-premium-lilac-light/30 shadow-premium-soft' : 'border-premium-lilac/20 bg-white/70 hover:bg-white'}`}
                        >
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <ChevronDown
                                className={`h-4 w-4 text-premium-purple-plum/50 transition-transform ${isSelected ? 'rotate-180' : ''}`}
                              />
                            </div>
                          </div>
                          <p className="mt-2 text-sm text-premium-purple-plum/70">
                        ...
                        {isSelected && renderConsultationDetail(consultation)}
                      </div>
                    );
                  })}
                </div>
                        ...
                        {isExpanded && (
                          <div className="mt-3 space-y-2 border-t border-premium-lilac/15 pt-3">
                            {folder.consultations.map((consultation) => {
                              ...
                              return (
                                <div key={consultation.id} className="space-y-0">
                                  <button
                                    type="button"
                                    onClick={() => toggleConsultationSelection(consultation.id)}
                                    aria-expanded={isSelected}
                                    className={`w-full rounded-2xl border p-3 text-left transition-all ${isSelected ? 'border-premium-purple-plum bg-premium-lilac-light/30' : 'border-premium-lilac/15 bg-white hover:bg-premium-pearl-tint/40'}`}
                                  >
                                    <div className="flex flex-wrap items-center justify-between gap-3">
